...
export default function MelhoresFretesPage() {
  ...
  function verificarLimite(): { pode: boolean; motivo?: string; restantes: number } {
    if (!profile) return { pode: false, motivo: 'Carregando...', restantes: 0 }
    ...
    if (isPro) {
      ...
      if (restantes <= 0) return { pode: false, motivo: `Limite diário atingido (${LIMITE_PRO_DIA}/dia). Volta amanhã.`, restantes: 0 }
      ...
      if (restantes <= 0) return { pode: false, motivo: `Limite mensal atingido (${LIMITE_FREE_MES}/mês). Assine o Pro para 10 buscas por dia.`, restantes: 0 }
      return { pode: true, restantes }
    }
  }
  ...
  async function buscarFretes() {
    ...
    const isPro = profile.plano === 'active'
    const hoje = new Date().toISOString().slice(0, 10)
    const mesAtual = new Date().toISOString().slice(0, 10)
        ...
        <>
          ...
          {!limite.pode && (
            <div className="card" style={{ borderColor: 'color-mix(in srgb, var(--primary) 40%, transparent)', background: 'color-mix(in srgb, var(--primary) 7%, transparent)', textAlign: 'center', padding: '40px 24px' }}>
              ...
              {!isPro && <button className="btn btn-primary" onClick={() => router.push('/planos')}>Ver plano Pro →</button>}
            </div>
          )}
